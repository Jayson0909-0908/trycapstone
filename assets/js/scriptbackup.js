    ...
    this.registerEvents = function(){
        document.addEventListener('click',function(e){
                ...
                if (Object.keys(loadScript.orderItems).length) {
                    ...
                    let content = '\
                            <div class="checkoutTotalAmountContainer" style="text-align:center;margin-top:10px">\
                                <span style="font-size: 36px;font-weight: bold;color: #f55;">₱' + totalAmt + '</span><br>\
                                ...
                                <div class="summary-info" style="display:flex; justify-content: space-between; font-size: 18px; color: #444;">\
                                    <div>Total: ₱<span class="totalAmt">' + totalAmt + '</span></div>\
                        ...
                        onshown: function(dialogRef) {
                                    ...
                                    row.innerHTML = `
                                        ...
                                        <td>
                                            <div style="margin-top: 10px;">
                                                ...
                                                <input class="form-control tendered-input" type="number" placeholder="0.00" style="text-align: right; width: 60px; display: inline-block; margin-left: 5px; border: 1px solid #ccc; padding: 5px; font-size: 14px;">
                                            </div>
                                        </td>
                            ...
                            document.getElementById('cashless-btn').addEventListener('click', function() {
                                if (paymentMethodInput.value === 'cash') {
                                    ...
                                    row.innerHTML = `
                                        <td>
                                            Gcash number: <input type="number" id="contact-mix"  maxlength="11" class="form-control" />
                                    ...
                                    tbody.innerHTML = `
                                        <tr class="cashless-row">
                                            <td>
                                                ...
                                                Gcash number: <input type="number" id="contact-cashless" maxlength="11" class="form-control" />
                        ...
                        buttons: [
                            {
                                ...
                                // Combine methods into one string (e.g., 'cash/cashless')
                                    ...
                                    const contact = 
                                        paymentMethod === 'cashless'
                                            ? document.getElementById('contact-cashless').value
                                            : paymentMethod === 'cash and cashless'
                                            ? document.getElementById('contact-mix').value
                                            : null;
                                    ...
                                    if (paymentMethod === 'cashless' || paymentMethod === 'cash and cashless') {
                                        ...
                                        if (!contact.startsWith("09") || contact.length !== 11 || isNaN(contact)) {
                                            BootstrapDialog.alert({
                                                type: BootstrapDialog.TYPE_DANGER,
                                        ...
                                        // Ensure contact number doesn't exceed 11 digits
                                        if (contact.length > 11) {
                                            ...
                                            return; // Prevent checkout if contact number exceeds 11 digits
                                        }
                                    }
            ...
            function updateAmounts() {
                ...
                document.querySelector('.paymentsAmt').textContent = totalTendered.toFixed(2);
            
                if (totalTendered < totalAmount) {
                    document.querySelector('.balanceAmt').textContent = (totalAmount - totalTendered).toFixed(2);
                    document.querySelector('.changeAmt').textContent = '0.00';
                } else {
                    document.querySelector('.balanceAmt').textContent = '0.00';
                    document.querySelector('.changeAmt').textContent = (totalTendered - totalAmount).toFixed(2);
                }
            }
...
document.addEventListener("DOMContentLoaded", function () {
    const contactInput = document.getElementById("contact-mix");

    // Add input event listener to restrict input to only numbers and a max length of 11 digits
...
document.addEventListener("DOMContentLoaded", function () {
    const contactInput = document.getElementById("contact-cashless");
    ...
    console.log("Data reset and product click listeners reinitialized"); // Debugging purpose
        ...
        this.updateOrderItemTable = function () {
            ...
            if (Object.keys(loadScript.orderItems)) {
                ...
                let rowNum = 1;
                for (const [pid, orderItem] of Object.entries(loadScript.orderItems)) {
                    let productElement = document.querySelector(`#product-items .productContainer[data-pid="${pid}"]`);
                    if (!productElement) {
                        console.error(`Product with PID ${pid} not found.`);
                        continue; // Skip if product element is missing
                    }
        
                    let vatStatusElement = productElement.querySelector('.productVatStatus');
                    if (!vatStatusElement) {
                        console.error(`VAT status for PID ${pid} not found.`);
                        continue; // Skip if VAT status is missing
                    }
        
                    let vatAmount = 0; // Default VAT is 0
                    if (vatStatusElement.textContent.trim() === 'Vatable') {
            ...
            if (vatDisplayElement) {
                vatDisplayElement.textContent = `₱${loadScript.totalVat.toFixed(2)}`; // Display total VAT
            }
...
this.updateTotalOrderAmount = function () {
    ...
    if (discountValue > subtotal) {
        ...
        document.querySelector('.item_total--total').textContent = `₱${subtotal.toFixed(2)}`; // Reset total to subtotal
    ...
    // Update the displays
    document.querySelector('.item_total--discount').textContent = `(₱${discountValue.toFixed(2)})`; // Display the discount value
    document.querySelector('.item_total--total').textContent = `₱${finalTotal.toFixed(2)}`; // Display the final total
};
